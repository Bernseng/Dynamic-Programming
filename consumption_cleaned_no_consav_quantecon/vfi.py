...
import scipy.optimize as sco # for optimization in adjuster problem
...
def obj_adj(choices,t,p,x,inv_v_keep,inv_v_adj,par):
    ...
    return value_of_choice(t,c,d,p,x,inv_v_keep,inv_v_adj,par) - penalty 

@njit
def obj_adj1(choices,t,p,x,inv_v_keep,inv_v_adj,par):
    """ evaluate bellman equation adjusters """
    ...
    return -value_of_choice(t,c,d,p,x,inv_v_keep,inv_v_adj,par) - penalty # changed to "-"" because of scipy.optimize.minimize
...
def solve_adj(t,sol,par):
    ...
    for i_p in prange(par.Np):
        ...
        for i_x in range(par.Nx):
                ...
                choices = results[0]
                minf = results[1]
                # print(f"i_p state: {i_p}, i_x state: {i_x}")
                # print(f'quantecon choices: {choices}, minf {minf}')

                # results = sco.minimize(obj_adj1,choices,
                #     method='Nelder-Mead',
                #     bounds = np.array([[1e-8,x],[1e-8,x]]), 
                #     args=(t,p,x,sol.inv_v_keep,sol.inv_v_adj,par),
                #     tol=par.tol, 
                ...
                # choices = results.x 
                # minf = results.fun 
                # print(f'scipy choices: {choices}, minf {minf}')

                # c. optimal values
                c[i_p,i_x] = choices[0]
                d[i_p,i_x] = choices[1]  
                inv_v[i_p,i_x] = -1/minf 
                #inv_v[i_p,i_x] = 1/minf # if using sco.minimize