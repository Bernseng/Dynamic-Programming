...
def lifecycle(sim,sol,par):
    ...
    for t in range(par.T):
        for i in prange(par.simN):
            ...
            optimal_choice(t,p[t,i],n[t,i],b[t,i],discrete[t,i:],d[t,i:],c[t,i:],a[t,i:],sol,par)
            
@njit            
def optimal_choice(t,p,n,b,discrete,d,c,a,sol,par):

    x = trans.x_plus_func(b,n,par)

    # a. discrete choice
    inv_v_keep = linear_interp.interp_3d(par.grid_p,par.grid_n,par.grid_b,sol.inv_v_keep[t],p,n,b)
    ...
    if adjust:
        ...
        tot = d[0]+c[0]
        if tot > x + par.omega[t]*d[0]: 
            d[0] *= (x + par.omega[t]*d[0])/tot
            c[0] *= (x + par.omega[t]*d[0])/tot
            a[0] = -par.omega[t]*d[0]
        else:
            a[0] = x - tot - par.omega[t]*d[0]
        ...
        for t in range(par.T-1):
            ...
            if constrained:
                ...
                for ishock in range(par.Nshocks):
                    ...
                    optimal_choice(t+1,p_plus,n_plus,b_plus,discrete_plus,d_plus,c_plus,a_plus,sol,par)
