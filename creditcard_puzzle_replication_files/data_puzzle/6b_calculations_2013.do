	...
	* c. central tables
	...
	foreach var of local outvars {
		...
		collapse (mean) mean = `var'_rel ///
						...
						[aw=weight], by(puzzle)
		export excel using "output_`cutval'_2013.xls", sheet("`var'") sheetreplace firstrow(variables)
		restore 
		...
		collapse (mean) mean =`var'_rel ///
						...
						(p75) p75 = `var'_rel (p85) p85 = `var'_rel (p95) p95 = `var'_rel (p99) p99 = `var'_rel ///
						[aw=weight]
		export excel using "output_`cutval'_2013.xls", sheet("`var'_tot") sheetreplace firstrow(variables)
		restore 
		...
		collapse (mean) mean = `var'_rel ///
						...
						(p75) p75 = `var'_rel (p85) p85 = `var'_rel (p95) p95 = `var'_rel (p99) p99 = `var'_rel ///
						[aw=weight], by(year)
		export excel using "output_`cutval'_2013.xls", sheet("`var'_year") sheetreplace firstrow(variables)
		restore 
	...
	// d. shares
		...
		export excel using "output_`cutval'_2013.xls", sheet("shares_year") sheetreplace firstrow(variables)
		restore 
		
		preserve
		collapse (mean) puzzle_share borrower_share saver_share corner_share [aw=weight]
		export excel using "output_`cutval'_2013.xls", sheet("shares") sheetreplace firstrow(variables)
		restore 
